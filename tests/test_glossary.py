...
class SeparatorFlexibleMatchingTests(unittest.TestCase):
    """A hyphen is not a different term.

    Between the words of a multiword term, space, hyphen and non-breaking
    space are the same separator — the compounds in this field are written
    both ways in the same paper, and arXiv puts U+00A0 between a name and its
    year. The separator is flexible, not optional: `power law` must still not
    match `powerlaw`.
    """

    def count(self, source, text):
        return glossary._count_in_text(source, text)

    def test_a_spaced_term_matches_the_hyphenated_form(self):
        self.assertEqual(self.count('power law', 'the power-law mapping'), 1)

    def test_a_hyphenated_term_matches_the_spaced_form(self):
        self.assertEqual(self.count('bit-width', 'assign a bit width here'), 1)
        self.assertEqual(self.count('heavy-tailed', 'a heavy tailed spectrum'), 1)
        self.assertEqual(self.count('zero-shot', 'six zero shot tasks'), 1)

    def test_the_separator_is_not_optional(self):
        self.assertEqual(self.count('power law', 'powerlaw is one word'), 0)
        self.assertEqual(self.count('bit-width', 'bitwidth is one word'), 0)

    def test_a_non_breaking_space_is_a_separator(self):
        # arXiv writes "HQQ\u00a0(Badri and Shaji 2023)".
        self.assertEqual(self.count('HQQ (Badri', 'we reuse HQQ\u00a0(Badri 2023)'), 1)
        self.assertEqual(self.count('power law', 'the power\u00a0law exponent'), 1)

    def test_word_boundaries_still_hold(self):
        self.assertEqual(self.count('cat', 'category theory'), 0)
        self.assertEqual(self.count('fit', 'benefit of this'), 0)

    def test_punctuation_heavy_names_are_untouched(self):
        self.assertEqual(self.count('C++', 'written in C++ here'), 1)
        self.assertEqual(self.count('.NET', 'the .NET runtime'), 1)

    def test_repeated_separators_collapse(self):
        self.assertEqual(self.count('power law', 'power   law'), 1)

    def test_a_cjk_term_is_unaffected(self):
        self.assertEqual(self.count('양자화', '가중치 양자화를 적용한다'), 1)

    def test_selection_picks_up_the_hyphenated_form(self):
        """The end-to-end effect: the term reaches the chunk's table."""
        data = {'version': 2, 'high_frequency_top_n': 0,
                'applied_meta_hashes': {},
                'terms': [{'id': 'power law', 'source': 'power law',
                           'target': '멱법칙', 'category': 'concept',
                           'aliases': [], 'gender': 'unknown',
                           'confidence': 'high', 'frequency': 3,
                           'evidence_refs': [], 'notes': ''}]}
        picked = glossary.select_terms_for_chunk(
            data, 'To set the scale of the power-law mapping we calibrate it.')
        self.assertEqual([t['source'] for t in picked], ['power law'],
                         'the hyphenated form must still select the term, or '
                         'the sub-agent never learns the canonical rendering')
