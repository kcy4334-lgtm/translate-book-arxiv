...
def find_calibre_convert():
    ...
    The Windows installer does not add Calibre to PATH, so standard install
    locations must be probed explicitly. The result is cached — this is called
    once per output format, and each miss costs a process spawn.
    """
    if _CALIBRE_CACHE:
...
def _get_font_family_for_lang(lang):
    """Font-family CSS for a language.

    The data lives in layout.LANG_CONFIG so this module and merge_and_build.py
    cannot drift apart again -- they used to hold independent copies of the
    same table, and they had already diverged.
    """
    return layout.get_lang_config_loose(lang)['font_family_ebook']


def _get_pdf_font_for_lang(lang):
    """Bare font name for Calibre's --pdf-*-family options."""
    return layout.get_lang_config_loose(lang)['pdf_font']
...
"""
        ...
        # Insert CSS immediately BEFORE </head>. It used to go right after
        # <head>, which put it ahead of the template's own <style> -- so
        # every rule here lost the cascade and silently did nothing. The
        # two type properties it used to set were removed in the same
        # change, so making it effective cannot regress EPUB layout.
        # Callable replacement: a backslash in a font-family name would
        # otherwise be read as a regex escape (same reason as
        # merge_and_build.py's author_meta substitution).
        if re.search(r'</head\s*>', content, re.IGNORECASE):
            content = re.sub(r'(</head\s*>)',
                             lambda m: font_css + m.group(1),
...
def convert_html_with_calibre(html_file, output_file, format_type, timeout=600, lang="zh-CN", cover=None):
    ...
    try:
        ...
        # encoding= is required: Calibre writes UTF-8, and decoding it with the
        # Windows locale codepage raises UnicodeDecodeError inside
        # subprocess.run, before the returncode check can even run.
