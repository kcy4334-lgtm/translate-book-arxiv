...
# Between the words of a multiword term, a space, a hyphen and a non-breaking
# space are the same separator. The glossary says "power law"; the paper
# writes "power-law mapping"; they are one term. Not optional, though --
# "power law" must still not match "powerlaw".
_TERM_SEPARATOR_RE = re.compile(r'[-\s\u00a0]+')
_TERM_SEPARATOR = r'[-\s\u00a0]+'


def _flexible_separators(source):
    """re.escape(source), with its word separators made interchangeable."""
    parts = [re.escape(p) for p in _TERM_SEPARATOR_RE.split(source) if p]
    return _TERM_SEPARATOR.join(parts) if parts else re.escape(source)
...
def _count_in_text(source, text):
    ...
    if source.isascii():
        pattern = rf'(?<!\w){_flexible_separators(source)}(?!\w)'
...
def _is_first_use_gloss(inside):
    r"""Is this parenthesis a term's English, or part of the sentence?

    `메모리 사용량(GB)` is a unit and `사후 학습 양자화(PTQ)` introduces an
    abbreviation the text then keeps using; neither is a first-use gloss and
    neither may be deduplicated. A gloss spells the term out, so it carries a
    lowercase letter. Citations are excluded outright.
    """
    if not inside or _CITATION_RE.search(inside):
        return False
    if not any(c.islower() for c in inside):
        return False
    return len(inside.split()) <= 5


def dedupe_glosses(text):
    r"""Keep each term's English on its FIRST use and drop it afterwards.

    Every sub-agent sees one chunk, so each can only gloss the first use it
    can see. Told to annotate first use — which they must be, or the English
    leaves the book entirely — a term spanning ten chunks comes back glossed
    ten times. The floor belongs in the prompt; the ceiling belongs here,
    where the whole book is visible at once.

    Returns (new_text, removed).
    """
    seen, out, cursor, removed = set(), [], 0, 0
    for m in _GLOSS_RE.finditer(text):
        inside = ' '.join(m.group(1).split())
        if not _is_first_use_gloss(inside):
            continue
        key = inside.lower()
        if key not in seen:
            seen.add(key)
